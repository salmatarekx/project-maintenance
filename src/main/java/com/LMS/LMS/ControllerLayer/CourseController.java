...
public class CourseController {
    ...
    public ResponseEntity<String> updateCourse(@PathVariable Long id, @Valid @RequestBody UpdateCourseRequest request) {
        courseService.updateCourse(id, request.getCourseDTO(), request.getCurrentUser());
        return ResponseEntity.ok("Course updated successfully.");
    }

    @DeleteMapping("/deleteCourse/{id}")
    public ResponseEntity<String> deleteCourse(@PathVariable Long id, @Valid @RequestBody DeleteCourseRequest request) {
